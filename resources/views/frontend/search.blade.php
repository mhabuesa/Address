...
<head>
    ...
    <title>Search Property | Address</title>
    <!-- FAVICON -->
                        ...
                        <div class="detail-wrapper-body">
                            <div class="listing-title-bar">
                                <div class="text-heading text-left">
                                    <p><a href="{{route('index')}}">Home </a> &nbsp;/&nbsp; <span>Search</span></p>
                                </div>
                    ...
                    <div class="item col-lg-4 col-md-6 col-xs-12 landscapes sale">
                        <div class="project-single" data-aos="fade-up">
                            <div class="project-inner project-head">
                                <div class="homes">
                                    <!-- homes img -->
                                    <a href="{{route('single.property',$search->id)}}" class="homes-img">
                                        <div class="homes-tag button alt sale text-capitalize">For {{$search->status}}</div>
                                        <div class="homes-price">&#2547 {{$search->price}}</div>
                                        <img src="{{asset('uploads')}}/properties/preview/{{$search->preview}}" alt="home-1" class="img-responsive">
                                    </a>
                                </div>
                                <div class="button-effect">

                                    <a href="{{$search->video}}" class="btn {{Auth::guard('client')->user()?'popup-video popup-youtube':'show-reg-form modal-open'}}"><i class="fas fa-video"></i></a>
                                    <a href="{{route('single.property',$search->id)}}" class="img-poppu btn"><i class="fa fa-photo"></i></a>
                                </div>
                            </div>
                            <!-- homes content -->
                            <div class="homes-content">
                                <!-- homes address -->
                                <h3><a href="{{route('single.property',$search->id)}}">{{$search->title}}</a></h3>
                                <div class=" d-inline-block px-2 button alt featured">{{App\Models\Place::find($search->place)->name}}</div>
                                <p class="homes-address mb-3">
                                    <a href="{{route('single.property',$search->id)}}">
                                        <i class="fa fa-map-marker"></i><span>{{$search->location}}</span>
                                    </a>
                                </p>
                                <!-- homes List -->
                                <ul class="homes-list clearfix pb-3">
                                    <li class="the-icons">
                                        ...
                                        <span>{{$search->bedroom}} Bedrooms</span>
                                    </li>

                                    <li class="the-icons">
                                        <i class="flaticon-square mr-2" aria-hidden="true"></i>
                                        <span>{{$search->size}} sq ft</span>
                                    </li>
                                    <li class="the-icons">
                                        <i class="flaticon-car mr-2" aria-hidden="true"></i>
                                        <span>{{$search->garage}} Garages</span>
                                    </li>
                                    <li class="the-icons">
                                        <i class="fa-regular fa-building"></i>
                                        <span>{{$search->unit}} Unit</span>
                                    </li>
                                </ul>
